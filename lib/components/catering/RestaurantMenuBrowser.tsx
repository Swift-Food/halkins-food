  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const nextGroupName = visibleEntries[0].target.getAttribute("data-group-name");
        if (nextGroupName) {
        ...
        {categoriesLoading
          ...
          : categories.map((category) => (
              <button
                ...
                className={`flex-shrink-0 py-2.5 rounded-xl text-sm font-medium transition-all border flex min-h-16 flex-col items-center justify-center gap-0.5 leading-none ${
                  ...
                  {groupedItems.map((group) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative flex-shrink-0 whitespace-nowrap pb-3 pt-0.5 text-sm font-semibold transition-colors ${
                          isActive
                            ? "text-primary"
